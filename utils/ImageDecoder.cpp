...
int ImageDecoder::decode_packet(AVFrame *pFrame) {
    ...
    if (response < 0) {
        std::cout << "Error while sending a packet to the decoder: " << av_err2str(response) << std::endl;
        return response;
    }
    ...
    while (response >= 0) {
        ...
        if (response == AVERROR(EAGAIN) || response == AVERROR_EOF) {
            ...
            std::cout << "Error while receiving a frame from the decoder: " << av_err2str(response) << std::endl;
            return response;
        }
...
int main(){
    ...
    char *input_filename = "/home/amourao/O Revisionista Bloopers-LNiZG48b0W0.mp4";
