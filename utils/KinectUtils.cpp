...
ExtendedAzureConfig buildKinectConfigFromYAML(YAML::Node config) {
    ...
    if (!config["streaming_color_format"].IsDefined()) {
        std::cout << "WARNING: Missing key: \"streaming_color_format\"" << std::endl;
        std::cout << "Using default: K4A_IMAGE_FORMAT_COLOR_MJPG" << std::endl;
        device_config.color_format = K4A_IMAGE_FORMAT_COLOR_MJPG;
    } else {
        std::string streaming_color_format = config["streaming_color_format"].as<std::string>();
        if (streaming_color_format == "K4A_IMAGE_FORMAT_COLOR_BGRA32") {
            ...
            std::cerr << "Supported values are K4A_IMAGE_FORMAT_COLOR_BGRA32 and K4A_IMAGE_FORMAT_COLOR_MJPG"
                      << std::endl;
            ...
            std::cerr << "Invalid value for: \"streaming_color_format\": " << streaming_color_format << std::endl;
            std::cerr << "Supported values are K4A_IMAGE_FORMAT_COLOR_BGRA32 and K4A_IMAGE_FORMAT_COLOR_MJPG"
                      << std::endl;
    ...
    if (!config["streaming_color_resolution"].IsDefined()) {
        std::cout << "WARNING: Missing key: \"streaming_color_resolution\"" << std::endl;
        std::cout << "Using default: K4A_COLOR_RESOLUTION_720P" << std::endl;
        device_config.color_resolution = K4A_COLOR_RESOLUTION_720P;
    } else {
        std::string streaming_color_resolution = config["streaming_color_resolution"].as<std::string>();
        if (streaming_color_resolution == "K4A_COLOR_RESOLUTION_720P") {
            ...
            std::cerr << "Invalid value for: \"streaming_color_resolution\": " << streaming_color_resolution
                      << std::endl;
            std::cerr
                    << "Supported values are K4A_COLOR_RESOLUTION_OFF, K4A_COLOR_RESOLUTION_720P, K4A_COLOR_RESOLUTION_1080P, K4A_COLOR_RESOLUTION_1440P, K4A_COLOR_RESOLUTION_1536P,  K4A_COLOR_RESOLUTION_2160P and K4A_COLOR_RESOLUTION_3072P"
                    << std::endl;
    ...
    if (!config["streaming_depth_mode"].IsDefined()) {
        ...
        std::string streaming_depth_mode = config["streaming_depth_mode"].as<std::string>();
        if (streaming_depth_mode == "K4A_DEPTH_MODE_PASSIVE_IR") {
            ...
            std::cerr << "Invalid value for: \"streaming_depth_mode\": " << streaming_depth_mode << std::endl;
            std::cerr
                    << "Supported values are K4A_DEPTH_MODE_OFF, K4A_DEPTH_MODE_NFOV_2X2BINNED, K4A_DEPTH_MODE_NFOV_UNBINNED, K4A_DEPTH_MODE_WFOV_2X2BINNED, K4A_DEPTH_MODE_WFOV_UNBINNED, K4A_DEPTH_MODE_PASSIVE_IR"
                    << std::endl;
    ...
    if (!config["streaming_rate"].IsDefined()) {
        ...
        if (fps_str == "K4A_FRAMES_PER_SECOND_5") {
            ...
            std::cerr << "Invalid value for: \"streaming_rate\": " << fps_str << std::endl;
            std::cerr
    ...
    if (!config["wired_sync_mode"].IsDefined()) {
        ...
        if (wired_sync_mode == "K4A_WIRED_SYNC_MODE_MASTER") {
            ...
            std::cerr << "Invalid value for: \"wired_sync_mode\": " << wired_sync_mode << std::endl;
            std::cerr
                    << "Supported values are K4A_WIRED_SYNC_MODE_MASTER, K4A_WIRED_SYNC_MODE_SUBORDINATE, K4A_WIRED_SYNC_MODE_STANDALONE"
                    << std::endl;
    ...
    if (config["depth_delay_off_color_usec"].IsDefined()) {
        device_config.depth_delay_off_color_usec = config["depth_delay_off_color_usec"].as<int>();
    } else { ;
    ...
    if (config["subordinate_delay_off_master_usec"].IsDefined()) {
        device_config.subordinate_delay_off_master_usec = config["subordinate_delay_off_master_usec"].as<int>();
    } else { ;
        std::cout << "Using default subordinate_delay_off_master_usec = 0" << std::endl;
    }
