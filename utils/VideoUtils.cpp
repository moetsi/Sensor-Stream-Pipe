  ...
  for (uint y = 0; y < pFrame->height; y++) {
    for (uint x = 0; x < pFrame->width; x++) {
      pFrame->data[0][y * pFrame->linesize[0] + x] =
          frame.at<float>(y, x) * MAX_DEPTH_VALUE_8_BITS;
    }
  }
...
void prepareColorFrame(cv::Mat &frame, AVFrame *pFrame) {
  for (uint y = 0; y < pFrame->height; y++) {
    for (uint x = 0; x < pFrame->width; x++) {
      pFrame->data[0][y * pFrame->linesize[0] + x] =
          frame.at<cv::Vec3b>(y, x).val[0];
    }
  }
  for (uint y = 0; y < pFrame->height / 2; y++) {
    for (uint x = 0; x < pFrame->width / 2; x++) {
      pFrame->data[1][y * pFrame->linesize[1] + x] =
          frame.at<cv::Vec3b>(2 * y, 2 * x).val[1];
      pFrame->data[2][y * pFrame->linesize[2] + x] =
          frame.at<cv::Vec3b>(2 * y, 2 * x).val[2];
    }
  }
}
