...
- (void)session:(ARSession *)session didUpdateFrame:(ARFrame *)frame
{
    ...
    if (@available(iOS 14.0, *))
    {
        ...
        {
            CVPixelBufferRef depthBuffer = frame.sceneDepth.depthMap;
            CVPixelBufferRef confBuffer = frame.sceneDepth.confidenceMap;
            
            size_t width = CVPixelBufferGetWidth(depthBuffer);
            size_t height = CVPixelBufferGetHeight(depthBuffer);
            OSType type = CVPixelBufferGetPixelFormatType(depthBuffer);
            // 256 x 192
            // kCVPixelFormatType_DepthFloat32 is `fdep`
            type = CVPixelBufferGetPixelFormatType(confBuffer);
            // kCVPixelFormatType_OneComponent8 is L008
        }
    }
}
@end
...
iPhoneReader::iPhoneReader()
{
    ...
    @autoreleasepool
    {
        ...
        pImpl->delegate = [SessionDelegate new];
...
{
    // TODO: verify pending frame from ARSession thread
    // Should block until next frame is available so CPU usage is reasonable
    usleep(10000);
    return true;
}
