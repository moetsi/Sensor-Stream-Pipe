...
private:
  ...
  std::string write_pattern_;

public:
  SSPProcPointcloud(std::string host_port, std::string write_pattern);
  ...
  static void
  TranformationHelpersWritePointCloud(const k4a_image_t point_cloud_image,
...
#define PLY_ELEMENT_VERTEX "element vertex"
  ...
  static int Worker(std::shared_ptr<NetworkReader> reader,
    ...
    bool write_to_disk = !write_pattern.empty();
    ...
    try {
      ...
      while (reader->HasNextFrame()) {
        ...
        for (FrameStruct f : f_list) {
          ...
          if (f.camera_calibration_data.type == 0 && !calibration_set) {
        ...
        try {
          ...
          std::string i_formatted =
              std::string(6 - i_str.length(), '0') + i_str;
