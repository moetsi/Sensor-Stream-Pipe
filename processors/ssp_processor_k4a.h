...
class SSPProcK4A : public ISSPProcessor {
private:
...
public:
  SSPProcK4A(std::string host_port);
  ...
  static void PrintBodyInformation(k4abt_body_t body) {
    std::cout << "Body ID: " << body.id << std::endl;
    for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++) {
      k4a_float3_t position = body.skeleton.joints[i].position;
      k4a_quaternion_t orientation = body.skeleton.joints[i].orientation;
      printf(
          "Joint[%d]: Position[mm] ( %f, %f, %f ); Orientation ( %f, %f, %f, "
          "%f) \n",
  ...
  static void PrintBodyIndexMapMiddleLine(k4a::image body_index_map) {
